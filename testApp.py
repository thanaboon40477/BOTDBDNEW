    ...
    if yesno == "y":
        ...
        try:
            ...
            for i in range(sum_page+1):
                ...
                # save_excel_page_new(box, keyword, driver, start, finishPage)
                next_page = driver.find_element_by_xpath('//*[@id="next"]')
                next_page.click()
                startPage += 1
                time.sleep(8)
            finish = startPage - 1
            save_excel_page_new(box, keyword, start, finish)
            print(f"Completed {start} to {finish} :)\n")
        except:
            save_excel_page_new(box, keyword, start, startPage)
        ...
        try:
            ...
            save_excel_all_new(box, keyword)
